...
const (
	...
	SettingKeyAutoGroupGlobalMode            SettingKey = "auto_group_global_mode"             // 全局自动分组模式（0关闭/1模糊/2精确/3正则）
...
func (s *Setting) Validate() error {
	...
	case SettingKeyAutoGroupGlobalMode:
		if _, ok := ParseAutoGroupSettingValue(s.Value); !ok {
			return fmt.Errorf("auto group global mode must be one of 0, 1, 2, 3, true, false")
		}
		return nil
