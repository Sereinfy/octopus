...
func TestMatchingModelIDs(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "regex", group: model.Group{Name: "unused", MatchRegex: `^gpt-\d+$`}, mode: model.AutoGroupTypeRegex, want: []int{1}},
		{name: "empty regex falls back to exact", group: model.Group{Name: "claude-3"}, mode: model.AutoGroupTypeRegex, want: []int{3}},
	}
...
func TestAutoGroupValidation(t *testing.T) {
	...
	if err := GroupCreate(&model.Group{Name: "invalid-regex", MatchRegex: "("}, context.Background()); err == nil {
